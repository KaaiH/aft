...
class TM:
    ...
    def __init__(self, Q, Sigma, Gamma, delta, s, t, r, verbose=False,
        ...
        # Verify that Sigma does not contain the left endmarket or blank
        if '⊔' in Sigma:
            sys.exit("TM-Error: Blank symbol \'⊔\' should not be an element" +
                     " of Sigma")
        if '⊢' in Sigma:
            sys.exit("TM-Error: Left endmarket symbol \'⊢\' should not be an" +
                     " element of Sigma")
    ...
    @staticmethod
    def visualize(trace_input, trace):
        """
        ...
        tape = Tape(trace_input)
        print("Trace specified: " + trace)
        print("Input specified: " + trace_input)
        print("New tape:")
        print(tape)
        ...
        while idx_elements < len(trace_elements) - 1:
            ...
            print("Made transition using step: " + trace[idx_str:idx_str + 9])
            print("New tape: ")
            print(tape)
